...
# --------------------------
# 2. Split Dataset into Train (75%), Validation (15%), and Test (10%) (Without Data Leakage)
# --------------------------
total_samples = len(X)
...
# --------------------------
# 4. Train SVM Model (Adjusted for Better Generalization)
# --------------------------
svm_model = SVR(kernel='rbf', C=10, gamma='scale', epsilon=0.05)  # Reduced C, increased epsilon
...
joblib.dump(svm_model, os.path.join(save_path, "svm_forex_model.pkl"))
...
# --------------------------
# 7. Evaluate Model on Test Set (Final Evaluation)
# --------------------------
...
plt.title('Actual vs Predicted Close Price (SVM) - Test Set', fontsize=16)
...
plt.title('SVM Actual vs Predicted Close Prices (Train, Validation, Test) - SVM', fontsize=16)
plt.xlabel('Time Steps', fontsize=12)
plt.ylabel('Close Price', fontsize=12)
plt.legend(fontsize=12)
plt.grid(True, linestyle='--', alpha=0.7)
